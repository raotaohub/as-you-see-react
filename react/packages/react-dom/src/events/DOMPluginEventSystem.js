  ...
  if (!(rootContainerElement: any)[listeningMarker]) {
    ...
    allNativeEvents.forEach(domEventName => {
      ...
      if (domEventName !== 'selectionchange') {
        if (!nonDelegatedEvents.has(domEventName)) {
          listenToNativeEvent(domEventName, false, rootContainerElement); // 监听冒泡阶段的事件
        }
        listenToNativeEvent(domEventName, true, rootContainerElement); // 监听捕获阶段的事件
      }
    });
    const ownerDocument =
        ...
        if (nodeTag === HostRoot || nodeTag === HostPortal) {
          ...
          while (container !== null) {
            ...
            if (parentTag === HostComponent || parentTag === HostText) {
              node = ancestorInst = parentNode;//找到目标 fiber
              continue mainLoop;
            }
  ...
  while (instance !== null) {
    ...
    if (tag === HostComponent && stateNode !== null) {
      ...
      if (reactEventName !== null) {
        const listener = getListener(instance, reactEventName);//取出该Fiber上对应的事件监听函数
        if (listener != null) {
          listeners.push(

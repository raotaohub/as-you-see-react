  ...
  if (lastEffect !== null) {
    ...
    do {
      if ((effect.tag & flags) === flags) {
        if (enableSchedulingProfiler) {
          if ((flags & HookPassive) !== NoHookEffect) {
            markComponentPassiveEffectMountStarted(finishedWork); // 标记组件副作用挂载【开始】
          } else if ((flags & HookLayout) !== NoHookEffect) {
        ...
        if (enableSchedulingProfiler) {
          if ((flags & HookPassive) !== NoHookEffect) {
            markComponentPassiveEffectMountStopped();// 标记组件副作用挂载【结束】
          } else if ((flags & HookLayout) !== NoHookEffect) {
            markComponentLayoutEffectMountStopped();
          }
        }
...
function commitLayoutEffectOnFiber(
  ...
  switch (finishedWork.tag) {
    ...
    case SimpleMemoComponent: {
      ...
      if (flags & Update) {
        commitHookLayoutEffects(finishedWork, HookLayout | HookHasEffect); // 传入对应 useLayoutEffect 的flags
      }
      break;
    }
    case ClassComponent: {
...
function getHostSibling(fiber: Fiber): ?Instance {console.log('[getHostSibling]');

...
// The root we're working on 正在处理的fiberRoot
let workInProgressRoot: FiberRoot | null = null;
// The fiber we're working on 正在处理的 fiber
let workInProgress: Fiber | null = null;
// The lanes we're rendering 工作中的渲染信道
...
//
// Most things in the work loop should deal with workInProgressRootRenderLanes. 在 work loop 层面 workInProgressRootRenderLanes
// Most things in begin/complete phases should deal with renderLanes. 在begin/complete层面会使用 renderLanes
export let renderLanes: Lanes = NoLanes;

// Whether to root completed, errored, suspended, etc. 正在处理的fiberRoot的工作结束时的状态
...
let rootDoesHavePassiveEffects: boolean = false;// root 已有副作用
let rootWithPendingPassiveEffects: FiberRoot | null = null;// 用于root的副作用
...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  ...
  // Check if any lanes are being starved by other work. If so, mark them as 检查是否有任何车道被其他工作所淹没。如果是这样，请将它们标记为
  // expired so we know to work on those next. 已过期，以便我们知道接下来要处理这些内容。
  markStarvedLanesAsExpired(root, currentTime);

  // Determine the next lanes to work on, and their priority. 确定下一个要处理的信道及其优先级。
  ...
  // We use the highest priority lane to represent the priority of the callback. 获得下个任务的回调优先级，
  const newCallbackPriority = getHighestPriorityLane(nextLanes);

  // Check if there's an existing task. We may be able to reuse it. 检查是否存在现有任务。我们也许可以重复使用它。feature: automatic batched update
  ...
  if (existingCallbackNode != null) {
    // Cancel the existing callback. We'll schedule a new one below. 取消现有回调。我们将在下面安排一个新的。
    cancelCallback(existingCallbackNode);
  }
  ...
  if (newCallbackPriority === SyncLane) {
    ...
    if (root.tag === LegacyRoot) {
      ...
      scheduleLegacySyncCallback(performSyncWorkOnRoot.bind(null, root)); // LegacyRoot 调度同步回调
    } else {
      scheduleSyncCallback(performSyncWorkOnRoot.bind(null, root)); // 调度同步回调
    }
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (exitStatus !== RootInProgress) {
    ...
    if (exitStatus === RootDidNotComplete) {
      ...
      // The render completed. render 完成
  ...
  ensureRootIsScheduled(root, now());
  if (root.callbackNode === originalCallbackNode) {
    // The task node scheduled for this root is the same one that's 为此根计划的任务节点与当前执行的节点相同。
    // currently executed. Need to return a continuation. 需要返回被打断的，以延续
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  // If the root or lanes have changed, throw out the existing stack 如果workInProgressRootRenderLanes已更改，请丢弃现有堆栈
  // and prepare a fresh one. Otherwise we'll continue where we left off.并准备新的workInProgressRootRenderLanes的。否则，我们将继续我们离开的地方。
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (next === null) {
    ...
    workInProgress = next; // 继续在workLoopSync中递进
  }
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    const siblingFiber = completedWork.sibling; // 指针交给下个兄弟fiber节点
    if (siblingFiber !== null) {
      // If there is more work to do in this returnFiber, do that next.
      workInProgress = siblingFiber; // 下个兄弟fiber节点存在，则结束当前 unitWork
      return;
    }
    // Otherwise, return to the parent 否则，请返回父级
    completedWork = returnFiber;
    // Update the next thing we're working on in case something throws. 更新我们正在处理的下一件事，以防万一有什幺东西抛出。
    workInProgress = completedWork;
  } while (completedWork !== null);// 以上 深度优先遍历

  // We've reached the root. 找到根节点
  if (workInProgressRootExitStatus === RootInProgress) {
    workInProgressRootExitStatus = RootCompleted; // 标记root的退出状态为 完成了
  }
}
...
function commitRoot(
  ...
  const previousUpdateLanePriority = getCurrentUpdatePriority();console.log('[commitRoot]');
  const prevTransition = ReactCurrentBatchConfig.transition;
...
function commitRootImpl(
  ...
  if (root === workInProgressRoot) {
    // We can reset these now that they are finished. 现在，我们可以重置一些render阶段使用的全局变量
    workInProgressRoot = null;
    workInProgress = null;
    workInProgressRootRenderLanes = NoLanes;
  } else {
  ...
  // Check if there are any effects in the whole tree. 检查整棵树中是否有任何影响。
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    commitMutationEffects(root, finishedWork, lanes);console.log('[commitMutationEffects]');
    ...
    commitLayoutEffects(finishedWork, root, lanes);console.log('[commitLayoutEffects]');
    if (__DEV__) {
      if (enableDebugTracing) {
        logLayoutEffectsStopped();
      }
  ...
  if (rootDoesHavePassiveEffects) {console.log('[rootDoesHavePassiveEffects]',rootDoesHavePassiveEffects);
...
function flushPassiveEffectsImpl() {
  ...
  commitPassiveUnmountEffects(root.current);//useEffect【卸载】
  commitPassiveMountEffects(root, root.current, lanes, transitions);//useEffect【挂载】

...
export function processUpdateQueue<State>(
  ...
  let pendingQueue = queue.shared.pending; // 还未处理的任务
  if (pendingQueue !== null) {
    queue.shared.pending = null; // 把循环链表剪掉

    // The pending queue is circular. Disconnect the pointer between first
    // and last so that it's non-circular.
    const lastPendingUpdate = pendingQueue;// 拿到尾
    const firstPendingUpdate = lastPendingUpdate.next; // 拿到链头
    lastPendingUpdate.next = null; // 剪掉
    // Append pending updates to base queue 将挂起的更新追加到基本队列
    if (lastBaseUpdate === null) {

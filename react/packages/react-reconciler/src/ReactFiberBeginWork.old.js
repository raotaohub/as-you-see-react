...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  cloneUpdateQueue(current, workInProgress);
  processUpdateQueue(workInProgress, nextProps, null, renderLanes); // 处理 update updateQueue shareQueue
  ...
  if (supportsHydration && prevState.isDehydrated) {
    ...
    reconcileChildren(current, workInProgress, nextChildren, renderLanes); // 根据ReactElement对象,生成Fiber子fiber
  }
  return workInProgress.child;
}
...
function beginWork(
  ...
  if (current !== null) {
    ...
    if (
      ...
      if (
        ...
        // 如果这是错误或悬念边界的第二次传递，则
        // 可能不会在“当前”上安排工作，因此我们检查此标志。
        (workInProgress.flags & DidCapture) === NoFlags
      ) {
        // No pending updates or context. Bail out now. 没有挂起的更新或上下文。现在就纾困。
        didReceiveUpdate = false;
  ...
  // Before entering the begin phase, clear pending update priority. 在进入开始阶段之前，请清除挂起的更新优先级。
  // TODO: This assumes that we're about to evaluate the component and process
  // the update queue. However, there's an exception: SimpleMemoComponent
  // sometimes bails out later in the begin phase. This indicates that we should
  // move this assignment out of the common path and into each branch.
  workInProgress.lanes = NoLanes;// 不能复用, 创建新的fiber节点

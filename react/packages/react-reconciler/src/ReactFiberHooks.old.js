...
// the work-in-progress hook. 等同于 workInProgress
...
let currentHook: Hook | null = null;//指向current.memoizedState
let workInProgressHook: Hook | null = null;//workInProgress.memoizedState
...
function pushEffect(tag, create, destroy, deps: Array<mixed> | void | null) {
  ...
  if (componentUpdateQueue === null) {
    ...
    if (lastEffect === null) {
      ...
      const firstEffect = lastEffect.next; // 取出头【1】
      lastEffect.next = effect;// 最后1个下一个指向 新创建的 effect【2】
      effect.next = firstEffect;// 新 effect 的下一个指向 刚刚取出的头【1】 【3】
      componentUpdateQueue.lastEffect = effect;//再把最后一个指向新 effect 【4】 整个过程就是将 新effect插入到最前端，然后重新连接成循环链表
    }
  }
  return effect;
}
...
function updateEffectImpl(
  ...
  hook.memoizedState = pushEffect(
    HookHasEffect | hookFlags, // 挂载有有副作用的flag
    create,
    destroy,
    nextDeps,
  );
  ...
  if (isRenderPhaseUpdate(fiber)) {
    ...
    if (
      ...
      if (lastRenderedReducer !== null) {
        ...
        try {
          const currentState: S = (queue.lastRenderedState: any); // 最后1次渲染器的结果

...
const PerformedWork =
/*                */
0b000000000000000000000001; // You can change the rest (and add more).
...
/*               */
0b000000000011111111111111; // These are not really side effects, but we still reuse this field.
...
const Forked =
/*                       */
0b000010000000000000000000; // Static tags describe aspects of a fiber that are not specific to a render,
...
const PassiveStatic =
/*                */
0b010000000000000000000000; // Flag used to identify newly inserted fibers. It isn't reset after commit unlike `Placement`.

const PlacementDEV =
/*                 */
0b100000000000000000000000; // Groups of flags that are used in the commit phase to skip over trees that
...
// eslint-disable-line no-undef
// eslint-disable-line no-undef
const getPublicInstance = $$$hostConfig.getPublicInstance;
const getRootHostContext = $$$hostConfig.getRootHostContext;
const getChildHostContext = $$$hostConfig.getChildHostContext;
const prepareForCommit = $$$hostConfig.prepareForCommit;
const resetAfterCommit = $$$hostConfig.resetAfterCommit;
...
const createTextInstance = $$$hostConfig.createTextInstance;
const scheduleTimeout = $$$hostConfig.scheduleTimeout;
const cancelTimeout = $$$hostConfig.cancelTimeout;
const noTimeout = $$$hostConfig.noTimeout;
const isPrimaryRenderer = $$$hostConfig.isPrimaryRenderer;
const warnsIfNotActing = $$$hostConfig.warnsIfNotActing;
const supportsMutation = $$$hostConfig.supportsMutation;
const supportsPersistence = $$$hostConfig.supportsPersistence;
const supportsHydration = $$$hostConfig.supportsHydration;
const getInstanceFromNode = $$$hostConfig.getInstanceFromNode;
const beforeActiveInstanceBlur = $$$hostConfig.beforeActiveInstanceBlur;
const afterActiveInstanceBlur = $$$hostConfig.afterActiveInstanceBlur;
const preparePortalMount = $$$hostConfig.preparePortalMount;
const prepareScopeUpdate = $$$hostConfig.prepareScopeUpdate;
const getInstanceFromScope = $$$hostConfig.getInstanceFromScope;
const getCurrentEventPriority = $$$hostConfig.getCurrentEventPriority;
const detachDeletedInstance = $$$hostConfig.detachDeletedInstance;
const requestPostPaintCallback = $$$hostConfig.requestPostPaintCallback; // -------------------
//      Microtasks
//     (optional)
// -------------------

const supportsMicrotasks = $$$hostConfig.supportsMicrotasks;
const scheduleMicrotask = $$$hostConfig.scheduleMicrotask; // -------------------
//      Test selectors
//     (optional)
// -------------------

const supportsTestSelectors = $$$hostConfig.supportsTestSelectors;
const findFiberRoot = $$$hostConfig.findFiberRoot;
const getBoundingRect = $$$hostConfig.getBoundingRect;
const getTextContent = $$$hostConfig.getTextContent;
const isHiddenSubtree = $$$hostConfig.isHiddenSubtree;
const matchAccessibilityRole = $$$hostConfig.matchAccessibilityRole;
const setFocusIfFocusable = $$$hostConfig.setFocusIfFocusable;
const setupIntersectionObserver = $$$hostConfig.setupIntersectionObserver; // -------------------
//      Mutation
//     (optional)
// -------------------
...
const cloneInstance = $$$hostConfig.cloneInstance;
const createContainerChildSet = $$$hostConfig.createContainerChildSet;
const appendChildToContainerChildSet = $$$hostConfig.appendChildToContainerChildSet;
const finalizeContainerChildren = $$$hostConfig.finalizeContainerChildren;
const replaceContainerChildren = $$$hostConfig.replaceContainerChildren;
const cloneHiddenInstance = $$$hostConfig.cloneHiddenInstance;
const cloneHiddenTextInstance = $$$hostConfig.cloneHiddenTextInstance; // -------------------
//     Hydration
//     (optional)
// -------------------

const canHydrateInstance = $$$hostConfig.canHydrateInstance;
const canHydrateTextInstance = $$$hostConfig.canHydrateTextInstance;
const canHydrateSuspenseInstance = $$$hostConfig.canHydrateSuspenseInstance;
const isSuspenseInstancePending = $$$hostConfig.isSuspenseInstancePending;
const isSuspenseInstanceFallback = $$$hostConfig.isSuspenseInstanceFallback;
const getSuspenseInstanceFallbackErrorDetails = $$$hostConfig.getSuspenseInstanceFallbackErrorDetails;
const registerSuspenseInstanceRetry = $$$hostConfig.registerSuspenseInstanceRetry;
const getNextHydratableSibling = $$$hostConfig.getNextHydratableSibling;
const getFirstHydratableChild = $$$hostConfig.getFirstHydratableChild;
const getFirstHydratableChildWithinContainer = $$$hostConfig.getFirstHydratableChildWithinContainer;
const getFirstHydratableChildWithinSuspenseInstance = $$$hostConfig.getFirstHydratableChildWithinSuspenseInstance;
const hydrateInstance = $$$hostConfig.hydrateInstance;
const hydrateTextInstance = $$$hostConfig.hydrateTextInstance;
const hydrateSuspenseInstance = $$$hostConfig.hydrateSuspenseInstance;
const getNextHydratableInstanceAfterSuspenseInstance = $$$hostConfig.getNextHydratableInstanceAfterSuspenseInstance;
const commitHydratedContainer = $$$hostConfig.commitHydratedContainer;
const commitHydratedSuspenseInstance = $$$hostConfig.commitHydratedSuspenseInstance;
const clearSuspenseBoundary = $$$hostConfig.clearSuspenseBoundary;
const clearSuspenseBoundaryFromContainer = $$$hostConfig.clearSuspenseBoundaryFromContainer;
const shouldDeleteUnhydratedTailInstances = $$$hostConfig.shouldDeleteUnhydratedTailInstances;
const didNotMatchHydratedContainerTextInstance = $$$hostConfig.didNotMatchHydratedContainerTextInstance;
const didNotMatchHydratedTextInstance = $$$hostConfig.didNotMatchHydratedTextInstance;
const didNotHydrateInstanceWithinContainer = $$$hostConfig.didNotHydrateInstanceWithinContainer;
const didNotHydrateInstanceWithinSuspenseInstance = $$$hostConfig.didNotHydrateInstanceWithinSuspenseInstance;
const didNotHydrateInstance = $$$hostConfig.didNotHydrateInstance;
const didNotFindHydratableInstanceWithinContainer = $$$hostConfig.didNotFindHydratableInstanceWithinContainer;
const didNotFindHydratableTextInstanceWithinContainer = $$$hostConfig.didNotFindHydratableTextInstanceWithinContainer;
const didNotFindHydratableSuspenseInstanceWithinContainer = $$$hostConfig.didNotFindHydratableSuspenseInstanceWithinContainer;
const didNotFindHydratableInstanceWithinSuspenseInstance = $$$hostConfig.didNotFindHydratableInstanceWithinSuspenseInstance;
const didNotFindHydratableTextInstanceWithinSuspenseInstance = $$$hostConfig.didNotFindHydratableTextInstanceWithinSuspenseInstance;
const didNotFindHydratableSuspenseInstanceWithinSuspenseInstance = $$$hostConfig.didNotFindHydratableSuspenseInstanceWithinSuspenseInstance;
const didNotFindHydratableInstance = $$$hostConfig.didNotFindHydratableInstance;
const didNotFindHydratableTextInstance = $$$hostConfig.didNotFindHydratableTextInstance;
const didNotFindHydratableSuspenseInstance = $$$hostConfig.didNotFindHydratableSuspenseInstance;
const errorHydratingContainer = $$$hostConfig.errorHydratingContainer;
const isHydratableResource = $$$hostConfig.isHydratableResource;
const getMatchingResourceInstance = $$$hostConfig.getMatchingResourceInstance;
...
function mountClassInstance(workInProgress, ctor, newProps, renderLanes) {
  ...
  if (typeof instance.componentDidMount === 'function') {
    const fiberFlags = Update | LayoutStatic;
...
const NoFlags$1 =
/*   */
0b00000; // Represents whether effect should fire.

const HasEffect =
/* */
0b00001; // Represents the phase in which the effect (not the clean-up) fires.
...
let thenableIndexCounter = 0; // Used for ids that are generated completely client-side (i.e. not during
  ...
  if (usable !== null && typeof usable === 'object') {
    if (typeof usable.then === 'function') {
      ...
      switch (thenable.status) {
        ...
        default:
          {
            const prevThenableAtIndex = getPreviouslyUsedThenableAtIndex(index);

            if (prevThenableAtIndex !== null) {
              switch (prevThenableAtIndex.status) {
                case 'fulfilled':
                  {
                    const fulfilledValue = prevThenableAtIndex.value;
                    return fulfilledValue;
                  }

                case 'rejected':
                  {
                    const rejectedError = prevThenableAtIndex.reason;
                    throw rejectedError;
                  }
              ...
              // the next attempt.
              trackUsedThenable(thenable, index); // Suspend.
...
function mountEvent(callback) {
  ...
  mountEffectImpl(Update, Snapshot$1, () => {
...
function updateEvent(callback) {
  ...
  updateEffectImpl(Update, Snapshot$1, () => {
    ref.current = callback;
  }, [ref, callback]);
...

{
  ...
  HooksDispatcherOnMount.getCacheForType = getCacheForType; // $FlowFixMe[escaped-generic] discovered when updating Flow
...
{
  HooksDispatcherOnMount.useEvent = mountEvent;
}
...
function stopProfilerTimerIfRunningAndRecordDelta(fiber, overrideBaseTime) {
  ...
  if (profilerStartTime >= 0) {
    const elapsedTime = now$1() - profilerStartTime; // $FlowFixMe[unsafe-addition] addition with possible null/undefined value
...
let workInProgressRootRenderLanes = NoLanes; // When this is true, the work-in-progress fiber just suspended (or errored) and
...
function prepareFreshStack(root, lanes) {
  ...
  if (workInProgress !== null) {
    let interruptedWork = workInProgressIsSuspended ? workInProgress : workInProgress.return;
...
//# sourceMappingURL=react-reconciler.profiling.min.js.map

...
function createResponseState(identifierPrefix, nonce, bootstrapScriptContent, bootstrapScripts, bootstrapModules) {
  ...
  if (bootstrapScripts !== undefined) {
    for (let i = 0; i < bootstrapScripts.length; i++) {
      ...
      const integrity = typeof scriptConfig === 'string' ? undefined : scriptConfig.integrity;
...
function trackSuspendedWakeable(wakeable) {
  ...
  const thenable = wakeable; // We use an expando to track the status and result of a thenable so that we
  ...
  if (usable !== null && typeof usable === 'object') {
    if (typeof usable.then === 'function') {
      ...
      switch (thenable.status) {
        ...
        default:
          {
            const prevThenableAtIndex = getPreviouslyUsedThenableAtIndex(thenableState, index);
...
function createRequest(children, responseState, rootFormatContext, progressiveChunkSize, onError, onAllReady, onShellReady, onShellError, onFatalError) {
  ...
  const rootTask = createTask(request, null, children, null, rootSegment, abortSet, emptyContextObject, rootContextSnapshot, emptyTreeContext);
...
function renderSuspenseBoundary(request, task, props) {
  ...
  const suspendedFallbackTask = createTask(request, null, fallback, parentBoundary, boundarySegment, fallbackAbortSet, task.legacyContext, task.context, task.treeContext);
...
function renderHostElement(request, task, type, props) {
  ...
  const children = pushStartInstance(segment.chunks, request.preamble, type, props, request.responseState, segment.formatContext);
...
function renderIndeterminateComponent(request, task, prevThenableState, Component, props) {
  ...
  const value = renderWithHooks(request, task, prevThenableState, Component, props, legacyContext);
...
function renderForwardRef(request, task, prevThenableState, type, props, ref) {
  ...
  const children = renderWithHooks(request, task, prevThenableState, type.render, props, ref);
  const hasId = checkDidRenderIdHook();
...
function spawnNewSuspendedTask(request, task, thenableState, x) {
  ...
  segment.lastPushedText = false;
  const newTask = createTask(request, thenableState, task.node, task.blockedBoundary, newSegment, task.abortSet, task.legacyContext, task.context, task.treeContext);
